KEYWORD_GROUPS = [
    {
        "translation": "Jeśli ogłoszenie wspomina o \"kulturze wysokiej wydajności\", może to oznaczać, że pracownicy są narażeni na wypalenie zawodowe. Większość ludzi chciałaby być częścią wysokowydajnego zespołu, ale rzeczywistość jest taka, że potrzeba silnego lidera, aby zrobić to dobrze. Ukryte oczekiwanie związane z tym zwrotem to: „robimy co trzeba, aby wykonać pracę, niezależnie od kosztów”. Zapytaj, jak firma wspiera pracowników w osiąganiu tych wyników i co się dzieje, gdy ktoś ma gorszy miesiąc.",
        "phrases": [
            "kultura wysokiej wydajności",
            "wysoka kultura pracy",
            "wysokiej wydajności",
            "środowisko wysokich wyników",
            "wysokich wyników",
            "high performance",
            "high-performance",
            "nastawienie na wyniki",
            "orientacja na wyniki",
            "wyniki ponad wszystko",
            "wyniki się liczą",
            "liczą się tylko wyniki",
            "kultura ciągłego doskonalenia",
            "kultura efektywności",
            "środowisko nastawione na sukces",
            "wymagające środowisko pracy",
            "ambitne środowisko",
            "kultura doskonałości",
            "performance driven",
            "performance-driven",
            "osiąganie wyników",
            "ukierunkowanie na wyniki",
            "meritokracja",
            "rezultaty mają znaczenie",
            "wyniki są najważniejsze",
            "skupienie na rezultatach",
            "result-oriented",
            "result driven",
            "drive for results",
            "delivery focused",
            "delivery-driven",
            "output oriented",
            "output-based",
            "wyniki przede wszystkim",
            "performance oriented",
            "ciągłe podnoszenie poprzeczki",
            "podnoszenie poprzeczki",
            "wymagamy wyników",
            "oczekujemy wyników",
            "kultura odpowiedzialności za wynik",
            "wynik to podstawa",
            "efektywność przede wszystkim",
            "highest standards",
            "zero tolerance for mediocrity",
            "nie akceptujemy przeciętności",
            "dążymy do perfekcji",
            "perfekcjonizm",
            "perfekcyjność",
            "excellence driven",
            "pursuit of excellence",
        ],
    },
    {
        "translation": "Dobre relacje ze współpracownikami są ważne, a posiadanie przyjaciół w pracy może zwiększyć satysfakcję i szczęście. Jednak jeśli ogłoszenie twierdzi, że współpracownicy są jak rodzina, może to oznaczać, że firma oczekuje bezwarunkowej lojalności i pracy po godzinach. Bycie zgranym zespołem, który współpracuje i ufa sobie nawzajem, to jedno, ale gdy pracodawca używa zwrotów takich jak „jesteśmy tutaj jak rodzina” – zwykle oznacza to brak granic. Zapytaj jaki mają stosunek do pracy po godzinach.",
        "phrases": [
            "jesteśmy jak rodzina",
            "jak rodzina",
            "rodzinna atmosfera",
            "pracujemy jak rodzina",
            "rodzinne środowisko",
            "traktujemy się jak rodzinę",
            "family atmosphere",
            "family feeling",
            "family-like",
            "family oriented",
            "jesteśmy zgraną paczką",
            "zgraną paczką",
            "atmosfera jak w domu",
            "jesteśmy jedną rodziną",
            "rodzinny klimat",
            "poczucie przynależności jak w rodzinie",
            "nasz team to rodzina",
            "jesteśmy rodziną",
            "pracownicy to rodzina",
            "czuj się jak w domu",
            "praca w rodzinnej firmie (ale nie w sensie własności)",
            "kameralny zespół",
            "domowa atmosfera",
            "nieformalne relacje",
            "brak dystansu",
            "wszyscy się znają",
            "jesteśmy blisko",
            "team jak rodzina",
            "wspieramy się jak rodzina",
            "rodzinne wartości",
            "family business vibe",
            "jesteśmy dla siebie jak rodzina",
            "atmosfera bez spiny",
            "swobodna atmosfera",
            "luz w pracy",
        ],
    },
    {
        "translation": "Upoważnianie pracowników to modne obecnie sformułowanie, ale jeśli musisz komuś dać władzę, oznacza to, że nigdy jej nie miał. Często jest to przesunięcie władzy, które nigdy nie dochodzi do skutku. Dodatkowo, \"empowerment\" to słowo często nadużywane bez pokrycia. Aby naprawdę upoważnić pracowników, muszą oni mieć możliwość wyrażania obaw bez obawy przed zemstą i mieć wpływ na sposób swojej pracy. Warto zapytać o konkretny przykład decyzji, którą może podjąć pracownik samodzielnie, bez zgody przełożonego.",
        "phrases": [
            "we empower you",
            "dajemy ci władzę",
            "empowerment",
            "upoważniamy cię",
            "masz wpływ na swoją pracę",
            "autonomia w działaniu",
            "zarządzasz swoją pracą",
            "ty decydujesz",
            "pełna autonomia",
            "swoboda decyzyjna",
            "empowered team",
            "employee empowerment",
            "twój głos się liczy",
            "masz realny wpływ",
            "wolna ręka w działaniu",
            "decyzyjność",
            "podejmujesz decyzje",
            "jesteś gospodarzem swojej roli",
            "samodzielne decyzje",
            "pełne zaufanie",
            "zaufanie na starcie",
            "nie ma nadzoru",
            "action-oriented empowerment",
            "możesz działać jak własny boss",
            "jesteś jak przedsiębiorca w firmie",
            "intrapreneur",
            "intrapreneruship",
            "samodzielność decyzyjna",
        ],
    },
    {
        "translation": "Większość pracowników jest zajęta, a jeśli są szczęśliwi, będą wkładać wysiłek w to, co robią. Jednak gdy słowo \"zabiegany\" pojawia się w ogłoszeniu o pracę, zwykle nie jest to dobry znak. Zwykle oznacza to, że zespół jest niedofinansowany i przepracowany lub że ich obciążenie pracą jest źle zarządzane. Dowiedz się jak wygląda przydzielanie obowiązków – czy zdarza się, że ktoś robi za dwa etaty.",
        "phrases": [
            "busy",
            "hard-working",
            "pracowity",
        ],
    },
    {
        "translation": "Zwrot \"musi radzić sobie ze stresem\" jest czerwoną flagą. Wypalenie zawodowe i słabe zdrowie psychiczne to poważne problemy w wielu branżach – stres jest czymś, co organizacje powinny starać się redukować, a nie wymagać od pracowników, aby go znosili. Oferty pracy, które podkreślają odporność na stres, często sygnalizują, że środowisko pracy jest źle zorganizowane, a presja jest nieodłącznym elementem codzienności. Spytaj, czy firma coś robi, żeby stres u pracownika chociaż zminimalizować minimalizować.",
        "phrases": [
            "must handle stress",
            "odporność na stres",
            "praca pod presją",
            "pod presją",
            "praca pod presją czasu",
            "pod presją czasu",
            "presja czasu",
            "umiejętność pracy w warunkach niepewności",
            "w warunkach niepewności",
            "odporność psychiczna",
            "radzenie sobie ze stresem",
            "zachowanie spokoju pod presją",
            "zimna krew w sytuacjach stresowych",
            "zimna krew",
            "umiejętność pracy w stresie",
            "tolerancja na stres",
            "praca pod dużą presją",
            "silna psychika",
            "praca w wymagających warunkach",
            "high pressure",
            "high-pressure",
            "stressful environment",
            "umiejętność działania pod presją",
            "odporność na trudne sytuacje",
            "praca w trudnych warunkach",
            "wysoka odporność na stres",
            "wysoka kultura pracy pod presją",
            "presja nie jest ci straszna",
            "nie boisz się presji",
            "presja działa na ciebie mobilizująco",
            "praca na pełnych obrotach",
            "non-stop w gotowości",
            "nerwy ze stali",
            "stalowe nerwy",
            "odporność na frustrację",
            "radzenie sobie z przeciwnościami",
            "praca w ogniu",
            "praca na pierwszej linii",
            "high-stakes environment",
            "umiejętność pracy w kryzysie",
            "praca w sytuacjach kryzysowych",
            "ciśnienie",
            "wytrzymałość psychiczna",
            "mental toughness",
            "resilience",
            "highly demanding environment",
        ],
    },
    {
        "translation": "Warto zwrócić uwagę, czy w ofercie jest jasno podane wynagrodzenie. Brak tej informacji może czasem oznaczać, że firma nie ma do końca przejrzystego podejścia do kwestii finansowych lub dopiero szacuje budżet. Podobnie, jeśli zarobki są uzależnione wyłącznie od efektów, dobrze sprawdzić, czy istnieje też jakaś stała podstawa, w przeciwnym razie może to prowadzić do presji lub wypalenia, zwłaszcza gdy cele są trudne do osiągnięcia. Dobrą praktyką jest zapytać rekrutera o widełki lub medianę wynagrodzeń w zespole, ponieważ to pomoże ocenić, czy oferta jest uczciwa i przejrzysta.",
        "phrases": [
            "wynagrodzenie uzależnione od efektów",
            "atrakcyjny system prowizyjny",
            "system prowizyjny",
            "prowizja bez górnego limitu",
            "bez górnego limitu",
            "zarobki adekwatne do zaangażowania",
            "wynagrodzenie do uzgodnienia",
            "do uzgodnienia",
            "wynagrodzenie zależne od wyników",
            "nieograniczone zarobki",
            "zarabiasz tyle ile chcesz",
            "twoje zarobki zależą od ciebie",
            "pensja do negocjacji",
            "do negocjacji",
            "wynagrodzenie do omówienia",
            "do omówienia",
            "atrakcyjne wynagrodzenie",
            "konkurencyjne wynagrodzenie",
            "wynagrodzenie dopasowane do kandydata",
            "płaca do uzgodnienia",
            "zarobki do rozmowy",
            "możliwości zarobkowe",
            "wysokie prowizje",
            "nieograniczony potencjał zarobkowy",
            "system premiowy za wyniki",
            "wynagrodzenie adekwatne",
            "rynkowe wynagrodzenie",
            "wynagrodzenie premiowe",
            "premia uznaniowa",
            "system bonusowy",
            "bonus uzależniony od wyników",
            "wynagrodzenie zadaniowe",
            "stawka + prowizja",
            "niska podstawa + wysoka prowizja",
            "twój sukces to twoja pensja",
            "zarabiaj tyle, na ile cię stać",
            "potencjalnie wysokie zarobki",
            "widełki: 1000 - 20000",
            "wynagrodzenie od do",
            "dochód nieograniczony",
            "zarobki bez górnej granicy",
            "skaluj swoje zarobki",
            "wynagrodzenie motywacyjne",
            "wynagrodzenie efektywnościowe",
        ],
    },
    {
        "translation": "To język, który opisuje kandydata jako osobę ambitną, elastyczną, myślącą nieszablonowo i działającą perswazyjnie. Badanie wykazało, że taki język nieproporcjonalnie przyciąga osoby z cechami narcystycznymi. Co kluczowe, osoby te są postrzegane jako mające większą skłonność do nieetycznych zachowań (np. manipulacja wynikami finansowymi), choć jednocześnie są bardziej innowacyjne. Nie każde użycie tego języka jest złe – problem pojawia się, gdy dominuje on w ogłoszeniu, zwłaszcza na stanowiskach wymagających zgodności z przepisami. Sprawdź, czy firma ma jasne procedury i czy ich przestrzeganie jest mile widziane, czy wręcz przeciwnie.",
        "phrases": [
            "Rule-Bender",
            "naginacz zasad",
        ],
    },
    {
        "translation": "Wymaganie skupienia na wielu zadaniach jednocześnie. Badania pokazują, że ludzki mózg nie jest przystosowany do prawdziwej wielozadaniowości – zamiast tego przełącza się między zadaniami, co obniża wydajność i zwiększa liczbę błędów. W ogłoszeniach o pracę może to oznaczać, że firma oczekuje od jednej osoby pracy na kilku stanowiskach lub że panuje tam chaos organizacyjny. Dowiedz się, czy chodzi o różnorodne projekty, czy raczej o robienie kilku rzeczy naraz z braku organizacji.",
        "phrases": [
            "wielozadaniowość",
            "multitasking",
            "multi-tasking",
            "umiejętność pracy na wielu frontach",
            "praca na wielu projektach jednocześnie",
            "obsługa wielu klientów jednocześnie",
            "jednoczesna praca nad wieloma projektami",
            "zdolność do równoległej pracy",
            "praca na kilku frontach",
            "praca równoległa",
            "przełączanie się między zadaniami",
            "task switching",
            "jednoczesna realizacja zadań",
            "kilka rzeczy naraz",
            "radzenie sobie z wieloma zadaniami",
            "umiejętność żonglowania zadaniami",
            "żonglowanie projektami",
            "praca na wielu wątkach",
            "jednoczesne prowadzenie kilku procesów",
            "równoczesna obsługa",
            "wielowątkowość",
            "równoległe realizowanie",
        ],
    },
    {
        "translation": "Może wskazywać na próbę zamaskowania braku profesjonalizmu i chaosu organizacyjnego. Firmy używające tych nieformalnych, często żartobliwych tytułów zamiast konkretnych nazw stanowisk, mogą nie mieć jasno określonych ścieżek kariery ani sprecyzowanych obowiązków. To również sposób na przyciągnięcie osób, które będą pracować z \"pasji\" zamiast za uczciwe wynagrodzenie. Zapytaj o konkretne obowiązki i ścieżkę kariery – nie daj się zwieść żartobliwym tytułom.",
        "phrases": [
            "rockstar",
            "ninja",
            "guru",
            "ewangelista",
            "wizard",
            "czarodziej",
            "superhero",
            "champion",
            "haker",
            "hacker",
            "jedi",
            "unicorn",
            "top performer",
            "superstar",
            "mastermind",
            "coding ninja",
            "code ninja",
            "marketing guru",
            "sales rockstar",
            "design wizard",
            "excel ninja",
            "product evangelist",
            "customer champion",
            "support superhero",
            "ops jedi",
            "growth hacker",
            "ninja developer",
            "rockstar developer",
            "guru of everything",
            "magician",
            "wunderkind",
            "czarodziej excela",
            "mistrz klawiatury",
            "ninja sprzedaży",
            "rockstar handlowca",
        ],
    },
    {
        "translation": "Warto podczas rozmowy zapytać, jak w praktyce wygląda zakres obowiązków – czy jest w miarę jasno określony, czy też firma oczekuje, że pracownik będzie robił różne rzeczy poza podstawowym zakresem. Dobrze upewnić się, jak często zdarzają się zadania spoza głównych obowiązków i jak wygląda ustalanie priorytetów. To pomoże ocenić, czy taka organizacja pracy będzie dla Ciebie komfortowa i czy nie grozi przeciążeniem.",
        "phrases": [
            "Other duties as assigned",
            "inne obowiązki zlecone przez przełożonego",
            "inne obowiązki według zlecenia",
            "inne obowiązki zlecone przez przełożonego",
            "inne obowiązki",
            "szeroki zakres obowiązków",
            "osoba do wszystkiego",
            "wszechstronność",
            "inne zadania wynikające",
            "oraz inne zadania",
            "i inne zadania",
            "dodatkowe zadania powierzone",
            "realizacja innych zadań",
            "różnorodne zadania",
            "zadania ad hoc",
            "ad hoc",
            "obsługa bieżących spraw",
            "inne zadania zgodnie z potrzebami",
            "zadania zlecone przez management",
            "różnorodny zakres działań",
            "praca na różnych polach",
            "elastyczny zakres obowiązków",
            "zadania nieujęte w opisie",
            "dodatkowe obowiązki",
            "i inne czynności",
            "wszystko co potrzebne",
            "pomoc przy innych projektach",
            "zadania wynikające z bieżących potrzeb",
            "wsparcie tam gdzie trzeba",
            "prace porządkowe",
            "działania nieprzewidziane",
            "obowiązki zmienne",
            "wypełnianie luk",
            "ogólne wsparcie operacyjne",
            "and other duties as assigned",
            "coś jeszcze może wyskoczyć",
            "praca w trybie wszędziepola",
            "generalista",
            "jack of all trades",
            "master of none",
            "szeroki zakres obowiązków",
            "osoba do wszystkiego",
            "wszechstronność",
        ],
    },
    {
        "translation": "Otwarcie przyznawany brak organizacji. Jeśli firma sama przyznaje, że panuje u niej chaos, należy potraktować to poważnie – to nie jest \"zaleta\", tylko problem do rozwiązania. Pracodawca, który oczekuje, że to pracownik będzie dostosowywał się do chaosu (zamiast go eliminować), prawdopodobnie nie inwestuje w procesy ani zarządzanie. Zapytaj, czy firma pracuje nad ograniczeniem chaosu.",
        "phrases": [
            "chaos",
            "chaotyczne środowisko",
            "dynamicznie zmieniające się otoczenie",
            "dynamicznie zmieniające",
            "praca w warunkach niepewności",
            "zarządzanie chaosem",
            "umiejętność pracy w chaosie",
            "dynamiczne środowisko",
            "szybko zmieniające się środowisko",
            "klimat startupowy",
            "startupowy",
            "startup atmosphere",
            "startup mindset",
            "startup culture",
            "szybkie zmiany",
            "ciągłe zmiany",
            "dynamicznie rozwijająca się firma",
            "środowisko w ciągłym ruchu",
            "szybko ewoluujące środowisko",
            "bardzo dynamiczne",
            "nieustanne zmiany",
            "praca w zmiennym środowisku",
            "tempo jak w startupie",
            "chaos produktywny",
            "kontrolowany chaos",
            "organiczny chaos",
            "nic nie jest stałe",
            "jutro może być inaczej",
            "zmiana to jedyna stała",
            "highly dynamic",
            "constantly evolving",
            "everything changes",
            "brak rutyny",
            "nigdy nie wiesz co przyniesie dzień",
            "work in ambiguity",
            "tolerancja na niejednoznaczność",
            "rada sobie w niejasnych sytuacjach",
            "nie ma sztywnych reguł",
            "wszystko płynie",
            "płynne środowisko",
        ],
    },
    {
        "translation": "Może oznaczać częste zmiany personalne, brak stabilizacji lub oczekiwanie, że pracownik będzie dostosowywał się do ciągle zmieniających się warunków bez odpowiedniego wsparcia. W zdrowych organizacjach \"dynamiczny\" oznacza elastyczność – w toksycznych oznacza chaos. Zapytaj o średni staż pracy w zespole.",
        "phrases": [
            "dynamiczny zespół",
            "dynamicznie",
        ],
    },
    {
        "translation": "Paradoksalnie, \"elastyczność\" w ogłoszeniach często nie oznacza korzyści dla pracownika (np. możliwości pracy zdalnej), lecz oczekiwanie, że to pracownik będzie dostosowywał się do potrzeb firmy – w tym pracy w nadgodzinach, w weekendy czy poza standardowym harmonogramem. Warto sprawdzić, co firma konkretnie rozumie przez \"elastyczność\".",
        "phrases": [
            "elastyczny czas pracy",
            "elastyczne godziny pracy",
            "elastyczne godziny zatrudnienia",
            "elastyczne godziny",
            "elastyczny grafik",
            "flextime",
            "nienormowany czas pracy",
            "nienormowany",
            "elastyczność w godzinach",
            "praca w elastycznych godzinach",
            "elastyczny wymiar czasu pracy",
            "możliwość dostosowania godzin",
            "praca bez sztywnych godzin",
            "bez sztywnych godzin",
            "godziny dopasowane do projektu",
            "elastyczne podejście do czasu pracy",
            "brak stałych godzin pracy",
            "brak stałych godzin",
            "elastyczność czasowa",
            "praca w różnych godzinach",
            "godziny uzależnione od projektu",
            "praca od poniedziałku do niedzieli",
            "dyspozycyjność czasowa",
            "dostępność poza biurem",
            "godziny pracy do uzgodnienia",
            "system zadaniowy (z nielimitowanym czasem)",
            "jesteśmy elastyczni",
            "dostosuj godziny do potrzeb firmy",
            "nierównomierne obciążenie godzinowe",
            "praca również w weekendy w zależności od potrzeb",
            "dostępność w godzinach wieczornych",
        ],
    },
    {
        "translation": "To klasyczny przykład \"fake benefitu\" – coś taniego i symbolicznego, co ma odwrócić uwagę od braku realnych korzyści, takich jak uczciwe wynagrodzenie, ubezpieczenie zdrowotne czy możliwość rozwoju. Jeśli firma chwali się owocowymi czwartkami, ale nie podaje widełek – to zły znak. Zapytaj o realne benefity – ubezpieczenie, budżet szkoleniowy, dodatkowe dni wolne.",
        "phrases": [
            "owocowe czwartki",
            "owocowe",
            "owoce w biurze",
            "owoce",
            "świeże owoce",
            "kawa i herbata",
            "strefa relaksu",
            "piłkarzyki",
            "stół do ping-ponga",
            "ping-pong",
            "ping pong",
            "imprezy integracyjne",
            "wyjazdy integracyjne",
            "integracyjne",
            "paczki świąteczne",
            "kawa na koszt firmy",
            "bezpłatna kawa",
            "praca w nowoczesnym biurze",
            "dostęp do kuchni",
            "chillout room",
            "chillout",
            "strefy odpoczynku",
            "konsola do gier",
            "gry planszowe",
            "słodycze w biurze",
            "piwo w piątek",
            "piwo w biurze",
            "darmowa herbata",
            "darmowe napoje",
            "lody w lecie",
            "pizza w piątek",
            "pizza friday",
            "śniadaniowe czwartki",
            "tłusty czwartek z pączkiem",
            "owocowe wtorki",
            "kuchnia wyposażona",
            "ekspres do kawy",
            "napój gazowane gratis",
            "imprezy co miesiąc",
            "owoce i warzywa",
            "zdrowa przekąska",
            "bilonówka",
            "stół do tenisa stołowego",
            "gaming room",
            "playstation w biurze",
            "xbox w pracy",
            "stoliki do piłkarzyków",
        ],
    },
    {
        "translation": "Może oznaczać brak doświadczonych pracowników, którzy mogliby wprowadzić procedury i kulturę szacunku. Często wiąże się z oczekiwaniem dyspozycyjności, pracy po godzinach i \"bycia częścią paczki\", gdzie granice między życiem zawodowym a prywatnym się zacierają. Może też sugerować wysoką rotację. Zapytaj, gdzie są starsi stażem pracownicy – jeśli ich nie ma, może to oznaczać wysoką rotację.",
        "phrases": [
            "młody zespół",
            "ambitny zespół",
            "praca w młodym gronie",
            "młodym gronie",
            "praca z młodymi ludźmi",
            "młodzi i dynamiczni",
            "energiczny zespół",
            "świeży zespół",
            "praca w młodym środowisku",
            "young team",
            "dynamiczny młody team",
            "entuzjastyczny zespół",
            "młodzi specjaliści",
            "passion-driven team",
            "zespół pełen energii",
            "grono młodych zapaleńców",
            "młodzi ambitni",
            "średnia wieku poniżej 30",
            "średnia wieku 25",
            "dwudziestokilkuletni zespół",
            "młodzieżowy vibe",
            "energia młodości",
            "early career team",
            "junior team",
            "świeżo po studiach",
            "młodzi wilcy",
            "młodzi i głodni sukcesu",
            "hungry team",
        ],
    },
    {
        "translation": "Oczekiwanie, że pracownik będzie dostępny w zasadzie przez cały czas. W zdrowych organizaczy takie wymagania są jasno określone, kompensowane (dodatkowy odpoczynek lub wynagrodzenie) i nie dotyczą codzienności. Brak normowania czasu pracy często oznacza, że firma nie liczy nadgodzin lub oczekuje ich za darmo. Dowiedz się, czy czas po godzinach jest rekompensowany.",
        "phrases": [
            "dyspozycyjność",
            "dyspozycyjność weekendowa",
            "weekendowa",
            "gotowość do wyjazdów służbowych",
            "wyjazdy służbowe",
            "stały kontakt",
            "dostępność pod telefonem",
            "pod telefonem",
            "bycie pod telefonem",
            "praca w weekendy",
            "praca w nadgodzinach",
            "gotowość do pracy w nadgodzinach",
            "nadgodziny",
            "możliwość pracy w niestandardowych godzinach",
            "brak ograniczeń czasowych",
            "dyspozycja",
            "gotowość na wyjazdy",
            "praca w systemie zmianowym",
            "system zmianowy",
            "praca nocna",
            "praca po godzinach",
            "kontakt poza godzinami",
            "dostępność 24/7",
            "24/7",
            "praca na zmiany",
            "praca w trybie on-call",
            "on-call",
            "dyżury telefoniczne",
            "dyżury w weekend",
            "praca w święta",
            "praca w niedziele",
            "praca w soboty",
            "dyspozycyjność w dni wolne",
            "dostępność w każdym momencie",
            "zawsze pod ręką",
            "gotowość do przyjazdu w każdej chwili",
            "gotowość operacyjna 24h",
            "stand-by",
            "reagowanie po godzinach",
            "praca na telefon",
            "bycie na baterii 24/7",
            "dostępność pod telefonem",
        ],
    },
    {
        "translation": "Może oznaczać, że firma działa chaotycznie, nie ma jasnych procesów, a pracownicy są pozostawieni sami sobie. W zdrowych organizacjach procedury istnieją po to, aby ułatwiać pracę – ich brak to nie \"swoboda\", tylko organizacyjny bałagan. Zapytaj, jak firma radzi sobie z niejsanymi sytuacjami.",
        "phrases": [
            "wyzwania zamiast procedur",
            " brak sztywnych ram",
            "brak korporacyjnych sztywnych zasad",
            "bez sztywnych zasad",
            "brak sztywnych zasad",
            "bez zasad",
            "naginanie zasad",
            "myślenie poza schematami",
            "przełamywanie granic",
            "łamanie zasad",
            "wychodzenie poza utarte schematy",
            "niestandardowe myślenie",
            "rule-bender",
            "disruptor",
            "disruption",
            "breaking the rules",
            "thinking outside the box",
            "outside the box",
            "myślenie out of the box",
            "out of the box",
            "poza schematami",
            "burzenie stereotypów",
            "kwestionowanie status quo",
            "rewolucyjne myślenie",
            "brak ograniczeń w myśleniu",
            "niekonwencjonalne podejście",
            "nie boimy się łamać zasad",
            "tu nie ma zasad",
            "reguły są po to, by je łamać",
            "nie ma rzeczy niemożliwych",
            "testujemy granice",
            "przesuwamy granice",
            "nie słuchamy procedur",
            "procedury nas nie ograniczają",
            "działamy niezgodnie ze schematem",
            "schematy są dla nudziarzy",
            "non-conformist",
            "rebel",
            "buntownik",
        ],
    },
    {
        "translation": "Często oznacza wysoką rotację – firma szybko awansuje, bo nie ma komu dać tych stanowisk, bo ludzie odchodzą. Może też oznaczać, że awans nie jest związany z realnymi kompetencjami, tylko z \"przetrwaniem\" lub dyspozycyjnością. Rzadko jest to oznaka stabilnej, zdrowej ścieżki kariery. Sprawdź, czy stanowiska nie zwalniają się przez rotację.",
        "phrases": [
            "możliwość szybkiego awansu",
            "szybki awans",
            "możliwość szybkiego awansu",
            "szybki awans",
            "szybki rozwój kariery",
            "szybka ścieżka awansu",
            "szybkie możliwości rozwoju",
            "szybki wzrost",
            "szybkie przejście na wyższe stanowisko",
            "przyspieszony awans",
            "możliwość awansu już po pierwszym roku",
            "awans za wyniki",
            "ekspresowy rozwój",
            "możliwość szybkiego wzrostu",
            "szybka ścieżka do zarządzania",
            "awans po 6 miesiącach",
            "szybka promocja",
            "od juniora do directora w rok",
            "dynamiczny awans",
            "wymarzona kariera w ekspresowym tempie",
            "szybko w górę",
            "zostań liderem w rok",
            "awans gwarantowany przy wynikach",
            "szybki awans dla ambitnych",
            "przyspieszona ścieżka menedżerska",
            "fast track",
            "fast promotion",
            "fast career growth",
        ],
    },
    {
        "translation": "W kontekście pracy, \"lojalność\" często oznacza oczekiwanie, że pracownik nie będzie odchodził, nie będzie narzekał i będzie pracował ponad standard, nawet jeśli firma nie daje mu nic w zamian. Zdrowa relacja pracodawca-pracownik opiera się na wzajemności, a nie na jednostronnej lojalności, więc zapytaj jak firma okazuje swoją lojalność.",
        "phrases": [
            "lojalność",
            "lojalny",
            "lojalny pracownik",
            "oczekujemy lojalności",
            "lojalność wobec firmy",
            "długoterminowa współpraca",
            "szukamy kogoś na lata",
            "chcemy kogoś na stałe",
            "poszukujemy lojalnych pracowników",
            "stałe zaangażowanie",
            "lojalność to nasza wartość",
            "przywiązanie do firmy",
            "oczekujemy oddania",
            "nie zmieniaj pracy co rok",
            "szukamy pracownika na lata",
            "etat na zawsze",
            "my tu jesteśmy rodziną i tak ma zostać",
            "nie szukamy osób na chwilę",
            "zależy nam na długofalowej współpracy",
            "wymagamy lojalności",
            "nie zatrudniamy freelancerów",
            "poszukujemy kogoś kto zostanie z nami",
            "budowanie razem",
            "wspólna droga",
            "razem przez lata",
            "kariera w jednej firmie",
        ],
    },
    {
        "translation": "Sugeruje, że normalna, przewidywalna praca jest w tej firmie niemożliwa – co oznacza, że panuje tam chaos, częste zmiany lub przeciążenie. Upewnij się, co oznacza ten termin w tej firmie: \"Brak nudy\" w zdrowym miejscu pracy oznacza ciekawe projekty – w toksycznym – wieczne gaszenie pożarów.",
        "phrases": [
            "brak nudy",
            "nie ma nudy",
        ],
    },
    {
        "translation": "Klasyczny red flag. Oznacza presję czasową, brak planowania i często niedobory kadrowe. W zdrowych organizacjach tempo jest dostosowane do możliwości zespołu. Zapytaj, z czego wynikają te szybkie terminy.",
        "phrases": [
            "praca na wczoraj",
            "szybkie terminy",
            "tight deadlines",
            "fast-paced",
            "szybko działający zespół",
            "praca pod time pressure",
            "time pressure",
            "krótkie terminy realizacji",
            "dynamiczne tempo",
            "intensywne tempo",
            "wymagające terminy",
            "praca w trybie pilnym",
            "tempo startupu",
            "high-paced",
            "agile pace",
            "krótkie deadliny",
            "deadline to świętość",
            "dotrzymywanie deadline'ów za wszelką cenę",
            "terminy się nie przesuwają",
            "pilne zadania na już",
            "reagowanie w trybie natychmiastowym",
            "natychmiastowa realizacja",
            "wysoka presja czasowa",
            "clock is ticking",
            "time-sensitive",
            "godzina jest ważna",
            "praca z minutnikiem",
            "zadania do zamknięcia w trybie ekspresowym",
            "fast delivery",
            "short turnaround",
            "szybki time-to-market",
            "praca w systemie just-in-time",
        ],
    },
    {
        "translation": "Z pozoru pozytywna cecha, ale w toksycznym kontekście może oznaczać, że firma nie zapewnia szkoleń, wsparcia ani feedbacku, a pracownik jest pozostawiony sam sobie z odpowiedzialnością, ale bez narzędzi czy decyzyjności. \"Swoboda działania\" bez jasnych granic i zasobów to pułapka. Zapytaj, czy samodzielność idzie w parze z dostępem do narzędzi i decyzyjności.",
        "phrases": [
            "samodzielność w działaniu",
            "swoboda działania",
            "praca bez nadzoru",
            "swoboda",
        ],
    },
    {
        "translation": "Te sformułowania chyba często służą do gaszenia uzasadnionej krytyki ze strony pracowników. Wskazują na kulturę, w której chyba nie można narzekać, zgłaszać problemów ani mówić o przeciążeniu – bo to podobno \"negatywne nastawienie\". Pracownik ma raczej \"szukać rozwiązań\" samodzielnie, nawet jeśli problem leży po stronie zarządzania. Dowiedz się, czy w firmie można zgłaszać problemy i krytykę, czy byłoby to uznawane za \"negatywne nastawienie\". Ważne jest jednak to, że jeśli praca dotyczy pracy z klientem, to \"pozytywne nastawienie\" w ofercie wcale nie musi być negatywnym sformułowaniem.",
        "phrases": [
            "szukanie rozwiązań nie problemów",
            "pozytywne nastawienie",
            "nie boimy się ciężkiej pracy",
            "podejście can-do",
            "can-do attitude",
            "can-do",
            "szukamy kogoś z drivem",
            "z drivem",
            "szukamy kogoś z iskrą",
            "z iskrą",
            "wyjście poza strefę komfortu",
            "poza strefę komfortu",
            "strefę komfortu",
            "proaktywna postawa",
            "szukamy optymistów",
            "pozytywna energia",
            "growth mindset",
            "mentalność wzrostu",
            "nastawienie na rozwiązania",
            "szukamy ludzi z inicjatywą",
            "własna inicjatywa",
            "szukamy kogoś z wewnętrznym motorem",
            "szukamy samodzielnych inicjatorów",
            "nie narzekamy, działamy",
            "skupiamy się na rozwiązaniach",
            "narzekanie nie jest wskazane",
            "zero narzekania",
            "brak miejsca na pesymizm",
            "nie oczekujemy marudzenia",
            "szukamy wojowników a nie maruderów",
            "pozytywny mindest",
            "energia i uśmiech",
            "nie ma złych nastrojów",
            "nie ma złych dni",
            "nie ma wymówek",
            "nie ma problemów są wyzwania",
        ],
    },
    {
        "translation": "Często oznacza, że podstawowe wynagrodzenie jest bardzo niskie lub żadne, a pracownik musi sam \"zarobić\" na swoje utrzymanie. Może prowadzić do niezdrowej rywalizacji i presji. Warto zawsze pytać o medianę zarobków na danym stanowisku.",
        "phrases": [
            "atrakcyjny system prowizyjny",
        ],
    },
    {
        "translation": "To sposób na uzasadnienie niskich płac – \"robisz to z pasji, nie dla pieniędzy\". Może oznaczać, że firma wykorzystuje zaangażowanie pracowników, nie oferując uczciwego wynagrodzenia. Praca może być pasją, ale pasja nie płaci rachunków. Zapytaj wprost, jak firma dba o work-life balance.",
        "phrases": [
            "praca dla pasjonatów",
            "szukamy pasjonatów",
            "pasjonaci",
            "pasja do pracy",
            "robimy to z pasją",
            "praca z pasją",
            "z pasją",
            "szukamy kogoś z pasją",
            "miłośnicy swojej pracy",
            "pokochaj swoją pracę",
            "praca to styl życia",
            "pasjonaci branży",
            "szukamy entuzjastów",
            "pasja zamiast rutyny",
            "praca to powołanie",
            "powołanie",
            "misja nie praca",
            "dołącz do ludzi z misją",
            "praca z zamiłowania",
            "pasjonat swojej dziedziny",
            "work is passion",
            "praca z sercem",
            "to więcej niż praca",
            "to nasze życie",
            "żyjemy tym",
            "oddajemy serce",
            "praca jako hobby",
            "hobby które stało się pracą",
            "jeśli to nie twoja pasja, nie pasujesz",
            "szukamy kogoś, kto żyje pracą",
            "praca jako misja życiowa",
            "nie szukamy pracowników, szukamy pasjonatów",
            "praca = spełnienie",
            "to nie tylko etat, to styl życia",
        ],
    },
    {
        "translation": "Firma oczekuje całkowitego podporządkowania życia zawodowi, traktując granicę między pracą a życiem prywatnym jako nieistniejącą. Objawia się to presją na pracę w godzinach nadliczbowych, w weekendy i święta, brakiem akceptacji dla zwolnień lekarskich (L4) oraz stygmatyzowaniem dbania o własne zdrowie. Pracownik ma być dostępny 24/7, a wszelkie formy odpoczynku lub choroby są postrzegane jako słabość lub brak zaangażowania. W praktyce prowadzi to do szybkiego wypalenia zawodowego i destrukcji życia prywatnego. Zapytaj wprost, jak firma dba o work-life balance.",
        "phrases": [
            "praca to pasja",
            "praca to życie",
            "żyjemy pracą",
            "nie odcinamy się",
            "firma jest ważna",
            "praca jest ważniejsza niż życie prywatne",
            "weekendy nie są wolne",
            "urlop opcjonalny",
            "nie bierzemy urlopów",
            "my nie chorujemy",
            "L4 dla słabych",
            "nie bierzemy L4",
            "praca w chorobie",
            "nie ma czegoś takiego jak choroba",
            "obowiązek ponad zdrowie",
            "etat ponad wszystko",
            "praca 24/7",
            "ciągle w pracy",
            "brak życia prywatnego",
            "życie to praca",
            "praca pochłania",
            "praca w święta to norma",
            "praca w weekend to norma",
            "nie ma wolnych weekendów",
            "jesteśmy dostępni zawsze",
            "nie ma zwolnień lekarskich",
            "praca w każdych warunkach",
        ],
    },
    {
        "translation": "Firma nie oferuje jasno określonej ścieżki awansu pionowego. Brak stanowisk, tytułów i hierarchii oznacza często, że pracownik nie ma możliwości rozwoju ani podwyżek wynikających z objęcia wyższego stanowiska – wszystko opiera się na \"uznaniowości\" lub wymagane jest przejście do innej firmy, by awansować. \"Otwarte drzwi\" i \"każdy mówi każdemu po imieniu\" mogą maskować chaos decyzyjny, brak odpowiedzialności oraz sytuację, w której stażysta ma taki sam głos jak dyrektor (ale bez realnej sprawczości). Jest to często pułapka dla osób ambitnych, które szybko odkrywają, że nie ma dokąd \"rosnąć\". Zapytaj, jak wygląda ścieżka awansu i podwyżki oraz czy w ogóle istnieją.",
        "phrases": [
            "płaska struktura",
            "brak hierarchii",
            "otwarte drzwi",
            "polityka otwartych drzwi",
            "brak barier",
            "horyzontalna struktura",
            "flat structure",
            "flat organization",
            "bez korporacyjnych zasad",
            "brak sztywnych procedur",
            "bez biurokracji",
            "bez korporacyjnej biurokracji",
            "non-hierarchical",
            "swobodna organizacja",
            "brak sztywnych ram",
            "bez sztywnych zasad",
            "każdy mówi każdemu po imieniu",
            "nie ma działów",
            "nie ma stanowisk",
            "brak tytułów",
            "wszyscy są równi",
            "kultura open door",
            "dostęp do każdego",
            "bez formalności",
            "bez dress code'u",
            "nie ma korpo-sztywności",
            "zero korpo",
            "brak szczebli",
            "jeden poziom",
            "wszyscy na tym samym poziomie",
            "każdy ma głos",
            "każdy może do każdego",
            "horyzontalna kariera",
            "brak barier",
        ],
    },
    {
        "translation": "To sformułowanie może czasem oznaczać pewną ostrożność firmy, zwłaszcza na początku współpracy. Być może chodzi o to, żeby pracownik najpierw poznał zespół i zasady pracy w biurze – często przez okres 3–6 miesięcy. Nie musi to od razu wynikać z braku zaufania, ale warto mieć świadomość, że w niektórych firmach późniejsza elastyczność bywa ograniczona (np. \"w tej chwili nie ma takiej opcji\", \"zespół woli być razem\"). Dla osób, którym zależy na rzeczywistej pracy zdalnej, dobrze będzie zapytać o konkretne warunki: po jakim czasie, na jakich zasadach i czy faktycznie pracownicy mogą z tej opcji skorzystać.",
        "phrases": [
            "możliwość pracy zdalnej po wdrożeniu",
            "praca zdalna po okresie próbnym",
            "hybrydowo po wdrożeniu",
            "zdalnie po adaptacji",
            "remote po onboardingu",
            "po onboardingu",
            "praca zdalna po adaptacji",
            "po wdrożeniu możliwość pracy zdalnej",
            "docelowo praca zdalna",
            "możliwość pracy hybrydowej po wdrożeniu",
            "home office po wdrożeniu",
            "praca zdalna po stażu",
            "zdalnie po próbnym",
            "możliwość pracy z domu po wdrożeniu",
            "po wdrożeniu",
            "home office po okresie próbnym",
            "praca zdalna po 3 miesiącach",
            "zdalnie od zaraz? tylko po wdrożeniu",
            "najpierw onboarding w biurze, potem ewentualnie zdalnie",
            "hybryda po okresie adaptacji",
            "zdalnie możliwe, ale nie od razu",
            "praca zdalna po zdobyciu zaufania",
            "elastyczność po czasie",
            "praca z domu dozwolona po wdrożeniu",
        ],
    },
    {
        "translation": "Warto docenić, że firma szuka osób zmotywowanych do rozwoju i samodzielnego uczenia się, to pozytywny sygnał. Jednocześnie dobrze jest podczas rozmowy sprawdzić, czy samodzielność w zdobywaniu wiedzy idzie w parze z jakimś podstawowym wsparciem ze strony firmy. Można zapytać, czy jest onboarding, dostęp do mentora, budżet szkoleniowy czy ktoś wyznaczony do pomocy na starcie. Wiele firm łączy zachętę do self-learningu z faktycznym wdrożeniem i wsparciem, dzięki czemu zachowany jest dobry balans. Jeśli jednak firma mówi wyłącznie o rzuceniu na głęboką wodę lub uczeniu się na własnych błędach bez żadnej struktury, warto mieć świadomość, że dla niektórych osób może to być trudne do ogarnięcia na początek.",
        "phrases": [
            "gotowość do nauki nowych rzeczy",
            "chęć do nauki",
            "self-learning",
            "samodzielne zdobywanie wiedzy",
            "ciągłe doskonalenie się",
            "umiejętność szybkiego uczenia się",
            "szybkie przyswajanie wiedzy",
            "silna motywacja do nauki",
            "stałe podnoszenie kwalifikacji",
            "nauka na bieżąco",
            "gotowość do ciągłego uczenia się",
            "otwartość na nowe technologie",
            "samodoskonalenie",
            "ciągły rozwój osobisty",
            "umiejętność uczenia się w biegu",
            "samodzielne szukanie rozwiązań",
            "własny development bez wsparcia",
            "żadnych szkoleń, tylko własna inicjatywa",
            "nie czekaj na gotowca, sam się naucz",
            "google to twój przyjaciel",
            "stackoverflow zamiast mentora",
            "docieramy w boju",
            "uczymy się na własnych błędach",
            "nie ma onboardingu, rzucamy na głęboką wodę",
            "samodzielne ogarnianie",
            "uczenie się na zadaniach",
            "praktyka zamiast teorii",
            "brak budżetu szkoleniowego",
            "nie ma mentora, jest wikipeda",
            "jeśli nie wiesz, sam się dowiedz",
        ],
    },
    {
        "translation": "Mgliste sformułowanie, które często oznacza \"dopasujesz się do nas\" lub \"będziesz spełniał nasze niewypowiedziane oczekiwania\". Może być używane do dyskryminacji lub wykluczania osób, które nie pasują do nieformalnych norm. Zapytaj, co to konkretnie oznacza, czy chodzi o wartości, czy o niewypowiedziane normy.",
        "phrases": [
            "wysoka kultura osobista",
            "dopasowanie do zespołu",
            "culture fit",
            "cultural fit",
            "pasowanie do kultury",
            "wartości zgodne z naszymi",
            "dzielenie wspólnych wartości",
            "zgodność z kulturą organizacyjną",
            "dopasowanie do wartości firmy",
            "poszukujemy kogoś kto pasuje",
            "odpowiednie nastawienie",
            "zgodność z naszą kulturą",
            "dopasowanie kulturowe",
            "musisz pasować",
            "nie każdy tu pasuje",
            "szukamy kogoś na swój vibe",
            "dopasowanie personalne",
            "musisz być jednym z nas",
            "my tu tworzymy wyjątkową kulturę",
            "nie dla każdego",
            "tylko dla wybranych",
            "jeśli nie pasujesz, nie aplikuj",
            "dopasowanie do naszych wartości",
            "wartości: tu wpisz cokolwiek",
            "kultura > kompetencje",
            "nastawienie > umiejętności",
        ],
    },
    {
        "translation": "Firma zrzuca na nową osobę odpowiedzialność za naprawę wszystkich procesów, kultury organizacyjnej czy wyników, ale nie daje jej do tego realnej władzy, budżetu, zespołu ani wsparcia ze strony kadry zarządzającej. Oczekuje się, że jedna osoba (często na średnim lub niższym stanowisku) dokona \"transformacji\" i \"reorganizacji\", podczas gdy reszta firmy działa po staremu, a przełożeni blokują zmiany lub nie podejmują decyzji. Jest to klasyczna pułapka – odpowiedzialność za sukces (lub porażkę) ponosi \"lider zmian\", ale narzędzia do wprowadzenia tych zmian pozostają poza jego zasięgiem. Często kończy się to szybkim wypaleniem i odejściem, a firma szuka kolejnego \"zbawcy\". Zapytaj, jakie zasoby i realną władzę decyzyjną będzie miał \"lider zmian\".",
        "phrases": [
            "lider zmian",
            "agent zmiany",
            "change agent",
            "change maker",
            "poszukiwany lider zmian",
            "inicjator zmian",
            "osoba która odmieni firmę",
            "katalizator zmian",
            "motorem zmian",
            "osoba napędzająca zmiany",
            "transformacja organizacyjna",
            "przeprowadzisz nas przez transformację",
            "zmienisz to co nie działa",
            "naprawisz procesy",
            "poprawisz to co złe",
            "wdrążysz nowe standardy",
            "odpowiadasz za zmiany",
            "masz wprowadzić nową jakość",
            "reorganizacja",
            "od nowa zbudujesz",
            "zmienisz kulturę organizacji",
            "zrewolucjonizujesz",
            "przekształcisz",
            "jesteś naszym zbawcą",
            "przyjdź i napraw",
            "naprawa firmy",
            "turnaround leader",
            "transformation leader",
            "change leadership",
            "reforma",
            "masz nas zmienić",
        ],
    },
    {
        "translation": "Może sugerować, że firma kładzie duży nacisk na zaangażowanie i identyfikację z celami organizacji – czasem kosztem wynagrodzenia lub work-life balance. Dobrze zapytać, jak to się przekłada na codzienne warunki i benefity.",
        "phrases": [
            "to więcej niż praca",
            "to nie jest zwykła praca",
            "szukamy misjonarzy",
            "praca jako misja",
            "nasza firma to misja",
            "zmieniamy świat",
            "jesteśmy na misji",
            "dołącz do misji",
            "praca z sensem",
            "praca która ma znaczenie",
            "praca która zmienia świat",
            "praca to służba",
            "służymy klientom",
            "mamy wyższy cel",
            "naszym celem jest coś więcej niż zysk",
            "jesteśmy tu dla czegoś więcej",
            "praca dla idei",
            "nie dla pieniędzy, dla idei",
            "misja ponad zysk",
            "cel nadrzędny",
            "działamy w słusznej sprawie",
            "ważna misja społeczna",
            "komercyjna misja",
            "cel > kasa",
            "wynagrodzenie drugorzędne",
            "liczy się misja a nie hajs",
        ],
    },
    {
        "translation": "Może oznaczać, że firma działa w szybkim tempie i nie wygospodarowuje przestrzeni na systematyczne wdrażanie nowych osób czy tworzenie dokumentacji. Warto sprawdzić, jak wygląda onboarding i kto będzie wsparciem na początku.",
        "phrases": [
            "nie mamy czasu na rozwój",
            "brak czasu na wdrożenie",
            "ciężko o czas na dokumentację",
            "non stop zadania",
            "wszyscy są zajęci",
            "nie ma mentora, bo każdy ma swoje zadania",
            "nikt nie ma dla ciebie czasu",
            "rzucamy na głęboką wodę",
            "przecież dasz radę sam",
            "wszyscy są zajęci swoimi projektami",
            "brak opieki mentorskiej",
            "brak czasu na szkolenia",
            "nie mamy budżetu czasowego na rozwój",
            "rozwój we własnym zakresie",
            "onboarding trwa dzień",
            "wszystkiego nauczysz się w praktyce",
            "tu nie ma czasu na teorię",
            "od razu do roboty",
            "liczy się działanie a nie nauka",
            "nie ma czasu na czytanie dokumentacji",
            "dokumentacja nie istnieje",
            "wiedza jest w głowach, nie na wiki",
            "baza wiedzy nie istnieje",
            "nie mamy luka na SOP",
        ],
    },
    {
        "translation": "Może sugerować, że atmosfera w zespole jest oparta na współzawodnictwie – to bywa motywujące, ale czasem prowadzi do napięć. Warto zapytać, jak firma dba o współpracę i czy premiowany jest też teamwork.",
        "phrases": [
            "zdrowa rywalizacja",
            "rywalizacja między pracownikami",
            "rankingi pracowników",
            "tablica wyników",
            "walczymy o premie",
            "tylko najlepsi dostają",
            "system rywalizacji",
            "walka o klienta",
            "walka o prowizję",
            "rywalizacja zespołowa",
            "zawody o wyniki",
            "kto lepszy",
            "tylko zwycięzcy",
            "konkurs na wyniki",
            "pracownicy rywalizują",
            "teamy konkurują ze sobą",
            "jeden wygrywa, reszta przegrywa",
            "system rankingu",
            "performance ranking",
            "sales battle",
            "sales competition",
            "wewnętrzna rywalizacja",
            "bitwa o prowizję",
            "nie ma team spirit, jest walka",
            "zdrowa rywalizacja (nigdy nie jest zdrowa)",
        ],
    },
    {
        "translation": "Może się okazać, że w firmie panuje dość dynamiczne (lub chaotyczne) zarządzanie zadaniami, a priorytety potrafią się szybko zmieniać. Warto upewnić się, jak wygląda komunikacja w zespole i kto ostatecznie decyduje o ważności zadań.",
        "phrases": [
            "wszystko priorytet",
            "każde zadanie pilne",
            "wszystkie terminy są ważne",
            "wiele priorytetów naraz",
            "priorytety się zmieniają",
            "dziś ważne, jutro nieważne",
            "ciągłe zmiany priorytetów",
            "nie wiadomo co najważniejsze",
            "lider zmienia zdanie co godzinę",
            "każde zadanie najważniejsze",
            "wszystko na wczoraj",
            "brak jasnych priorytetów",
            "ustalanie priorytetów to twój problem",
            "masz ogarnąć, co ważne",
            "wszystko jest ważne",
            "nie ma rzeczy mniej ważnych",
            "wszystkie projekty priorytetowe",
            "każdy klient priorytet",
            "każda sprawa pilna",
            "non stop zmiana priorytetów",
        ],
    },
    {
        "translation": "Z pozoru pozytywne cechy – ale mogą sugerować, że firma nie zapewnia ścisłych ram ani nadzoru, więc pracownik musi sam ogarniać swoje zadania i rozwój. Warto zapytać o dostępne wsparcie i procedury.",
        "phrases": [
            "umiejętność priorytetyzacji zadań",
            "umiejętność szybkiego uczenia się",
            "wysoka samodyscyplina",
        ],
    },
    {
        "translation": "Może sugerować, że mimo deklaracji autonomii, w praktyce funkcjonuje dość szczegółowa kontrola – np. raportowanie, rejestracja czasu. Warto doprecyzować, na czym polega ta kontrola i gdzie kończy się zaufanie.",
        "phrases": [
            "dajemy zaufanie, ale kontrolujemy",
            "zaufanie, ale weryfikacja",
            "pełne zaufanie, ale rozliczamy z każdego kroku",
            "jesteś wolny, ale musisz raportować",
            "autonomia, ale co godzina status",
            "możesz decydować, ale my zatwierdzamy",
            "samodzielność pod nadzorem",
            "jesteś odpowiedzialny, ale nie masz narzędzi",
            "zaufanie na dzień dobry, potem mikrozarządzanie",
            "jesteśmy jak rodzina, więc sprawdzamy każdy twój ruch",
            "sam planujesz, ale każdą zmianę konsultujesz",
            "elastyczność, ale tylko gdy pasuje do nas",
            "zespół się rozlicza",
            "daily standupy z rozliczeniem",
            "każda godzina rozliczona",
            "kontrola każdego zadania",
            "każda minuta pod lupą",
            "rejestracja czasu co do minuty",
            "kontrola aktywności",
            "monitoring pracy",
            "sprawdzamy co robisz",
            "wgląd w ekran",
            "audyt każdej czynności",
            "mikrozrządzenie",
            "micromanagement (nawet jak nie piszą wprost)",
        ],
    },
    {
        "translation": "Może sugerować, że środowisko pracy jest wymagające i nie każdy się w nim odnajdzie. To może być szczera informacja zwrotna – ale warto sprawdzić, czy chodzi o ambitne wyzwania, czy raczej o trudne warunki.",
        "phrases": [
            "nie dla każdego",
            "praca nie dla leniwych",
            "nie dla osób szukających łatwizny",
            "to nie jest praca dla każdego",
            "nie dla mięczaków",
            "praca dla twardzieli",
            "nie dla osób wrażliwych",
            "nie każdy to wytrzyma",
            "filtrujemy od razu",
            "jeśli nie jesteś gotowy na wyzwania, nie aplikuj",
            "nie dla ludzi którzy chcą mieć lekko",
            "praca nie dla osób po godzinie 16",
            "szukamy wojowników",
            "nie dla osób które chcą mieć łatwo",
            "to nie jest zwykła praca biurowa",
            "nie oczekujemy osób, które nie lubią wyzwań",
            "praca wymagająca charakteru",
            "nie dla każdego, tylko dla wybranych",
            "selekcja naturalna",
            "przetrwają najsilniejsi",
            "nie ma miejsca dla słabych",
            "nie ma litości",
            "hart ducha",
            "charakter trzeba mieć",
            "nie dla płaczków",
            "nie dla ludzi którzy się poddają",
            "nie dla osób które nie lubią presji",
        ],
    },
    {
        "translation": "Może sugerować, że firma szuka kogoś z dużym doświadczeniem, kto nie będzie potrzebował długiego wdrażania ani stałego wsparcia. Warto zapytać o dostępność mentora lub zespołu do konsultacji.",
        "phrases": [
            "samodzielny specjalista",
            "doświadczony i samodzielny",
            "nie potrzebuje wdrożenia",
            "szukamy gotowca",
            "kogoś od zaraz",
            "od ręki",
            "pracownik z własnym warsztatem",
            "nie oczekujemy wdrożenia",
            "zakładamy, że wszystko umiesz",
            "żadnego onboardingu",
            "mają być efekty od pierwszego dnia",
            "nie mamy czasu na naukę",
            "potrzebujemy kogoś na już",
            "wymagamy samodzielności",
            "zero wsparcia",
            "jesteś sam na placu boju",
            "nie ma buddy",
            "nie ma kogo pytać",
            "musisz sobie radzić sam",
            "oczekujemy pełnej autonomii od startu",
            "nie ma okresu wdrożeniowego",
            "ucz się na własnych błędach",
            "gotowiec na pozycję",
            "doświadczenie minimum 5 lat w każdej technologii",
            "mile widziane własne narzędzia",
            "przynosisz wiedzę i rozwiązania",
            "jesteś ekspertem, więc nie oczekuj pomocy",
        ],
    },
    {
        "translation": "Może sugerować, że w firmie często pojawiają się niestandardowe sytuacje lub brakuje sprawdzonych procedur – wtedy rzeczywiście potrzebna jest kreatywność. Warto zapytać, jakie problemy zdarzają się najczęściej.",
        "phrases": [
            "kreatywne podejście do problemów",
        ],
    },
    {
        "translation": "Cele to standard – ale mogą sugerować, że są one dość wyśrubowane i od ich realizacji wiele zależy. Warto przy rozmowie zapytać, jak wygląda wsparcie w osiąganiu celów i co się dzieje, gdy się ich nie zrealizuje.",
        "phrases": [
            "orientacja na cel",
            "ambitne cele sprzedażowe",
            "nastawienie na sukces",
        ],
    },
    {
        "translation": "Paradoksalnie – firmy, które naprawdę oferują stabilne zatrudnienie, rzadko to podkreślają. Jeśli ogłoszenie to akcentuje, może to oznaczać, że firma ma problem z rotacją i próbuje zachęcić do pozostania, ale nie poprzez dobre warunki, tylko poprzez obietnicę \"stabilności\". Warto zapytać o przeciętny staż pracy w zespole.",
        "phrases": [
            "stabilne zatrudnienie",
        ],
    },
    {
        "translation": "Często oznacza, że firma unika kosztów związanych z umową o pracę i może łatwo zakończyć współpracę. \"Okres próbny\" w ramach umowy zlecenie to oksymoron – ta forma zatrudnienia z definicji nie daje stabilności. Dobrze zapytać o docelową formę zatrudnienia i warunki przejścia.",
        "phrases": [
            "umowa zlecenie na okres próbny",
        ],
    },
    {
        "translation": "Może oznaczać, że firma nie ufa pracownikom – najpierw muszą \"zasłużyć\" na zaufanie, pracując pod bezpośrednim nadzorem. To dość powszechne, ale warto ustalić konkretne warunki i ramy czasowe.",
        "phrases": [
            "późniejsza możliwość pracy zdalnej",
            "możliwość pracy zdalnej po okresie wdrożenia",
            "późniejsza praca zdalna",
            "potem możliwość pracy zdalnej",
        ],
    },
    {
        "translation": "Może sugerować, że firma jest na wczesnym etapie rozwoju – często wiąże się to z brakiem procedur, długimi godzinami i mniejszą stabilnością, ale też z większą swobodą i szybkimi decyzjami. Warto ocenić, czy to pasuje do własnego stylu pracy.",
        "phrases": [
            "klimat startupowy",
            "startup",
            "jak w startupie",
            "startupowo",
        ],
    },
    {
        "translation": "Może oznaczać, że firma ma do czynienia z wymagającymi klientami i nie zawsze interweniuje w konfliktach – oczekuje samodzielności od pracownika. Warto zapytać o procedury wsparcia w trudnych sytuacjach.",
        "phrases": [
            "umiejętność radzenia sobie z trudnym klientem",
            "trudny klient",
            "trudni klienci",
        ],
    },
    {
        "translation": "Czasami może to oznaczać oczekiwanie pracy po godzinach bez rekompensaty. Pracownik, który daje 100%, jest już w pełni zaangażowany – oczekiwanie więcej może powodować wypalanie. Warto zapytać, jak oceniane i ważne jest faktyczne zaangażowanie.",
        "phrases": [
            "zaangażowanie 110%",
            "110%",
            "zaangażowanie po godzinach",
            "gotowość do poświęceń",
            "poświęcenia",
            "wkładamy serce w to co robimy",
            "praca z pełnym zaangażowaniem",
            "poświęcenie dla firmy",
            "extra mile",
            "going the extra mile",
            "ponad przeciętność",
            "100% zaangażowania",
            "pełne poświęcenie",
            "praca wykraczająca poza obowiązki",
            "above and beyond",
            "ponad obowiązki",
            "praca z sercem",
            "zaangażowanie bez kompromisów",
            "praca z pełną parą",
            "nie odmawiamy sobie niczego dla firmy",
            "firma na pierwszym miejscu",
            "praca przed życiem prywatnym",
            "wolimy pracować niż odpoczywać",
            "weekend? a co to?",
            "praca w święta? czemu nie",
            "krew, pot i łzy",
            "dajemy z siebie wszystko",
            "200% zaangażowania",
            "bezgraniczne zaangażowanie",
            "cały siebie dla firmy",
            "oddanie firmie",
            "lojalność bez granic",
            "praca przede wszystkim",
            "job comes first",
        ],
    },
    {
        "translation": "\"Wyzwania\" w zdrowym miejscu pracy to ambitne projekty i rozwój. W toksycznym – to radzenie sobie z chronicznymi problemami organizacyjnymi, którymi nikt nie chce się zająć. Warto sprawdzić na czym polegają te wyzwania.",
        "phrases": [
            "praca pełna wyzwań",
            "wyzwania",
        ],
    },
    {
        "translation": "Może sugerować, że w zespole panuje luźna atmosfera – ale warto zapytać, czy przypadkiem nie chodzi o to, by pracownik \"dobrze znosił\" nie zawsze komfortowe sytuacje czy żarty, zamiast je zgłaszać. Dobrze jest doprecyzować, jak firma radzi sobie z konfliktami.",
        "phrases": [
            "poczucie humoru",
        ],
    },
    {
        "translation": "Może sugerować stabilność i renomę. Czasem bywa jednak tak, że firmy z długą historią mają dość sztywną, biurokratyczną kulturę. Warto zapytać o tempo zmian i innowacyjność – czy firma idzie z duchem czasu.",
        "phrases": [
            "praca w firmie o ugruntowanej pozycji",
        ],
    },
    {
        "translation": "Być może w firmie panuje raczej bezpośrednia komunikacja – czasem dość otwarta i szczera, choć nie dla każdego komfortowa. W dobrze funkcjonującym środowisku feedback jest konstruktywny i podawany w sposób, który pomaga się rozwijać. Warto jednak zwrócić uwagę na sformułowania typu \"ostry feedback\", \"gruba skóra\" czy \"nie dla mięczaków\" – mogą one sugerować, że sposób przekazywania uwag bywa dosadny, co nie każdemu odpowiada, ale nie musi od razu oznaczać toksycznej kultury. Dobrze przy rozmowie zapytać, jak wygląda typowa sesja feedbackowa i czy firma szkoli przełożonych w udzielaniu informacji zwrotnej – to pomoże ocenić, czy ten styl komunikacji będzie dla Ciebie odpowiedni.",
        "phrases": [
            "odporność na krytykę",
            "przyjmowanie krytyki",
            "konstruktywna krytyka (jako przykrywka hejtu)",
            "otwartość na feedback",
            "informacja zwrotna",
            "feedback na każdym kroku",
            "ciągły feedback",
            "feedback w czasie rzeczywistym",
            "nie każdy wytrzyma nasz feedback",
            "u nas feedback jest ostry",
            "feedback bez owijania w bawełnę",
            "mówimy wprost",
            "bez owijania",
            "prosto w oczy",
            "radzenie sobie z uwagami",
            "gruba skóra",
            "nie bierz do siebie",
            "nie obrażaj się",
            "nie przejmuj się krytyką",
            "krytyka to rozwój",
            "jak nie umiesz przyjmować krytyki, nie pasujesz",
            "feedback jest surowy",
            "feedback nie dla mięczaków",
            "twarda szkoła feedbacku",
            "nie oczekujemy dyplomacji",
            "mówimy, co myślimy",
            "szczerość kosztem kultury",
        ],
    },
    {
        "translation": "W zdrowych organizacjach to naturalna cecha kultury – porażka jest lekcją, nie powodem do kary. Jeśli jednak firma musi to wyraźnie podkreślić w ogłoszeniu, warto przy rozmowie zapytać, jak faktycznie wygląda rozliczanie błędów i czy zdarzają się z tego tytułu negatywne konsekwencje.",
        "phrases": [
            "brak lęku przed porażką",
            "nie boimy się porażki",
            "bez konsekwencji za porażkę",
            "porażki się zdarzają",
        ],
    },
]
